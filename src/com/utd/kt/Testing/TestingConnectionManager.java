...
public class TestingConnectionManager {
	 ...
	 */
	private static int myId = TestingMain.myNodeId;

	public static boolean connectHigherNodes(SctpServerChannel serverChannel) {
		int totalNodes = TestingMain.mapNodes.size();
		int i = myId + 1;
		while (i <= totalNodes) {
			SctpChannel sctpChannel;
			try {
				sctpChannel = serverChannel.accept();
				TestingMain.connectionSocket.put(i, sctpChannel);
	...
	public static boolean connectLowerNodes() {
		...
		for (int i = 1; i < myId; i++) {
			...
			try {
				SctpChannel sctpChannel;
				InetSocketAddress Sa = new InetSocketAddress(TestingMain.mapNodes
						.get(i).getAddress(), TestingMain.mapNodes.get(i)
						.getPortNumber());
	...
	public static boolean createConnections() {
		...
		NodeDetails currentNode = TestingMain.mapNodes.get(myId);
	...
	public static void sendMessage(Message m, int destId) {
		...
		SctpChannel clientSocket = TestingMain.connectionSocket.get(destId);
